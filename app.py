...
from src.GuardarResultadosGaussJordan import guardar_resultado_gauss_jordan
from src.GuardarEnDBMetodos import guardar_resultado_completo
from src.HistorialLogica import obtener_historial_usuario
from src.PdfLogica import Greporte  # Generación de PDF
            ...
            if conn:
                try:
                    ...
                    if cursor.fetchone():
                        ...
                        # 1) Generar hash con PBKDF2-SHA256
        ...
        if accion == 'calcular':
            ...
            if matriz is None or matriz.shape[0] == 0:
                ...
                if mensaje:
                    ...
                    # soluciones es lista para plantilla
                    soluciones = soluciones.tolist()

        elif accion == 'guardar':
            matriz = parse_matriz(matriz_texto)
            if matriz is None or matriz.shape[0] == 0:
                error = "Formato de matriz inválido para guardar."
            else:
                soluciones, mensaje = gauss_jordan_logica(matriz)
                if mensaje:
                    error = mensaje
                    ...
                    resultado_id = guardar_resultado_gauss_jordan(usuario, matriz, soluciones)

                    if resultado_id:
                        return redirect(url_for('historial'))
                    else:
                        error = "Hubo un problema al guardar el resultado."
