...
def newton_raphsonLogica(funcion_str, x0, max_iter=4000, tol=1e-6, locals_dict=None):
    iteraciones = []
    mensaje = None
    ...
    try:
        ...
        funcion = sympify(funcion_str, locals=locals_dict if locals_dict else {})
    ...
    for i in range(1, max_iter + 1):
        try:
            ...
            if fx0 is None or dfx0 is None or np.isnan(fx0) or np.isnan(dfx0):
                mensaje = f"Función o derivada devolvió valor inválido en iteración {i}."
                return None, iteraciones, mensaje
...
# Los otros métodos que diste (secante, muller y gauss_jordan) están bien estructurados y no requieren cambios, aquí te los dejo integrados:
